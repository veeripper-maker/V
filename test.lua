...
local function teleportToCFrame(cframe)
    if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        local targetCFrame = cframe * CFrame.new(atmOffsetX, atmOffsetY, atmOffsetZ)
...
local function teleportToPresent(presentPart)
    if presentPart and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        local hrp = player.Character.HumanoidRootPart
        local targetCFrame = CFrame.new(presentPart.Position + Vector3.new(0, 5, 0))
        local distance = (hrp.Position - targetCFrame.Position).Magnitude
        local speed = 500
        if distance > 0 then
            local tweenInfo = TweenInfo.new(distance / speed, Enum.EasingStyle.Linear)
...
local function teleportToSpawn()
    ...
    if spawnLocation and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        local hrp = player.Character.HumanoidRootPart
        local targetCFrame = CFrame.new(spawnLocation.Position + Vector3.new(0, 3, 0))
        local distance = (hrp.Position - targetCFrame.Position).Magnitude
        local speed = 500
        if distance > 0 then
            local tweenInfo = TweenInfo.new(distance / speed, Enum.EasingStyle.Linear)
